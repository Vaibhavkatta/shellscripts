...
if ($Pocet>1){
  ...
  if ($errnum){
    if ($errnum==28){
      echo 'HEARTBEAT TIMED-OUT '.$Timeout.'SECS EXCEEDED.';
      exit(2);
    }else{
      echo "ERROR CONNECTING HOST!! Err:".curl_error($ch);
      exit(2);
    }
  }
  ...
  if($reqInfo['http_code'] != 200){
  	$Status=2;
  	$Msg.="HEARTBEAT NOT FOUND - HTTP ".$reqInfo['http_code'];
  }
  ...
  if (isset($Tc) and ($Time > $Tc)){
    $Status=2;
    $Msg.='HEARTBEAT '.$Time.'secs EXCEEDS TIME LIMIT '.$Tc.'secs';
  }elseif (isset($Tw) and ($Time > $Tw)){
    $Status=1;
    $Msg.='HEARTBEAT '.$Time.'secs EXCEEDS TIME LIMIT '.$Tw.'secs';
  }
  ...
  if (empty($Msg))$Msg='HEARTBEAT OK - ' .$Time.' secs';
